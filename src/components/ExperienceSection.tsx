import SectionBlock from "./SectionBlock";

const experiences = [
  {
    role: "Independent Software Developer",
    company: "Project-Based Work",
    year: "2023 – Present",
    description:
      "Building and deploying modern web applications, crypto tools, and interactive UI systems. Hands-on experience in full development lifecycle: design, frontend, backend integration, deployment, and optimization. Focused on scalable architecture, clean code, and real-world problem solving.",
    tech: [
      "React", 
      "Firebase", 
      "JavaScript (ES6+)", 
      "TypeScript", 
      "Tailwind CSS", 
      "Next.js", 
      "Three.js / 3D Web", 
      "Node.js", 
      "REST APIs", 
      "Git/GitHub", 
      "Vercel Deployment"
    ],
  },
  {
    role: "Crypto & Web3 Tool Builder",
    company: "Independent Projects",
    year: "2023 – Present",
    description:
      "Developed multi-chain wallet trackers, blockchain explorers, and token dashboards. Focused on web3 integration, real-time data fetching, and smooth UI/UX for users and developers.",
    tech: [
      "React", 
      "Sui Blockchain", 
      "Web3.js / Ethers.js", 
      "Node.js", 
      "REST APIs", 
      "Galxe / OKX API Integrations", 
      "Firebase", 
      "Tailwind CSS", 
      "Vercel Deployment"
    ],
  },
  {
    role: "eCommerce & Web Applications Developer",
    company: "Independent Projects",
    year: "2023 – Present",
    description:
      "Built fully functional eCommerce web apps, authentication, dynamic routing, and responsive UI.",
    tech: [
      "React", 
      "Redux / Context API", 
      "Firebase Auth & Firestore", 
      "Tailwind CSS", 
      "Stripe Payment Integration", 
      "Vercel Deployment", 
      "Responsive Web Design", 
      "Git/GitHub"
    ],
  },
];

const ExperienceSection = () => (
  <SectionBlock id="experience" title="Experience">
    <div className="space-y-12">
      {experiences.map((exp) => (
        <div
          key={exp.role}
          className="relative pl-8 md:pl-0 border-l md:border-l-0 border-black/20 md:grid md:grid-cols-[1fr_2fr] md:gap-8 pb-12 last:pb-0"
        >
          <div className="md:text-right md:pr-8 md:border-r border-black/20 relative">
            <div className="hidden md:block absolute top-1 -right-[5px] w-[9px] h-[9px] rounded-full bg-black"></div>
            <div className="md:hidden absolute top-1 -left-[5px] w-[9px] h-[9px] rounded-full bg-black"></div>

            
            <h3 className="font-bold text-lg md:text-xl">{exp.company}</h3>
          </div>

          <div className="mt-2 md:mt-0">
            <h3 className="text-lg font-bold text-foreground md:hidden mb-2">
              {exp.role}
            </h3>
            <h3 className="text-xl font-bold text-foreground hidden md:block mb-3">
              {exp.role}
            </h3>
            <p className="body-text text-sm md:text-base">{exp.description}</p>
          </div>
        </div>
      ))}
    </div>
  </SectionBlock>
);

export default ExperienceSection;
